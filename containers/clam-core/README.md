...
```bash
docker buildx build --platform linux/amd64 --load -t clam-core:0.1.1 containers/clam-core
```
...
```bash
docker image inspect clam-core:0.1.1 --format '{{.Os}}/{{.Architecture}}'
```
...
docker buildx build \
  ...
  --push \
  -t ghcr.io/aledavini7/clam-core:0.1.1 \
